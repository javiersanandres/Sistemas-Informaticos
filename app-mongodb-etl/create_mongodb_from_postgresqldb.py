...
def get_french_movies(conn):
    ...
    """
    query = (
        select(movies_table.c.movieid, movies_table.c.movietitle, movies_table.c.year)
        .join(countries_table, movies_table.c.movieid == countries_table.c.movieid)
        .where(countries_table.c.country == 'France')
    )
...
def get_related_movies(conn, movie_id, genres, most_related = True):
    ...
    base_query = (
        ...
        .group_by(movies_table.c.movieid, movies_table.c.movietitle, movies_table.c.year)  # Group by movie details
        .order_by(movies_table.c.year.desc())  # Order by most recent movies
    )

    if most_related:
        having_clause = func.count(genres_table.c.genre) == len(genres)  # Exact genre match
    else:
        having_clause = and_(
            func.count(genres_table.c.genre) >= len(genres) / 2,  # Minimum threshold
            func.count(genres_table.c.genre) < len(genres)        # Exclude full matches
        )
...
def create_mongo_documents():
    ...
    with engine.connect() as connection:
        ...
        for movie in french_movies:
            ...
            genres = [genre[0] for genre in connection.execute(genres_query).fetchall()]

            # Get all directors of the movie
            directors_query = (
                select(directors_table.c.directorname)
                .join(directors_info_table, directors_table.c.directorid == directors_info_table.c.directorid)
                .where(directors_info_table.c.movieid == movie_id)
            )
            directors = [director[0] for director in connection.execute(directors_query).fetchall()]

            # Get all actors of the movie
            actors_query = (
                select(actors_table.c.actorname)
                .join(actors_info_table, actors_table.c.actorid == actors_info_table.c.actorid)
                .where(actors_info_table.c.movieid == movie_id)
            )
            actors = [actors[0] for actors in connection.execute(actors_query).fetchall()]

            # Get most related movies based on genre coincidence
            most_related = get_related_movies(connection, movie_id, genres, most_related=True)

            # Get related movies based on genre coincidence
            related = get_related_movies(connection, movie_id, genres, most_related=False)
